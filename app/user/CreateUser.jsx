...
export default function CreateUser() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      //cookies
      document.cookie = `user_Long=${JSON.stringify(data)}; path=/; max-age=${
        60 * 60 * 24 * 300
      };`;
      ...
      let user_long = getCookie("user_Long");
      user_long = JSON.parse(user_long);
      setUser(user_long);
  ...
  return (
    <>
      <dataContext.Provider value={{ username, email, password }}>
        {notshow == true && <Navbar />}
        { user ? (
          <p className="mt-20 text-3xl">You are already signed in</p>
        ) : (
          <>
            <form
            ...
            >
              ...
              <label>
                ...
                />
                {check ? (
                  <span>Password should be more than 8 letters</span>
                ) : (
                  ""
                )}
              </label>
            ...
            <div>
              <p className="text-center">
                ...
                <Link href="/user/login" className="text-blue-400 text-center">
                  {" "}
                  Login
                </Link>
              </p>
